...
type PendingOnHitEffect =
  | { kind: "hook"; source: CardInstance; rulesCardIds: string[]; triggerCount: number }
...
export function pendingOnHitEffects(
  ...
  const addHook = (source: CardInstance): void => {
    ...
    const requestedTriggerCount = Math.max(
      ownHit ? ownScript?.onHitTriggerCount?.(ctx) ?? 1 : 0,
      inheritedHit ? inheritedScript?.onHitTriggerCount?.(ctx) ?? 1 : 0,
    );
    const triggerCount = Number.isSafeInteger(requestedTriggerCount) && requestedTriggerCount > 0
      ? requestedTriggerCount
      : 1;
    effects.push({
      kind: "hook",
      source,
...
export function queueHitTriggers(state: GameStateInternal,
  ...
  for (const effect of pendingOnHitEffects(state, runtime, link)) {
    if (effect.kind === "hook") {
      const { source, triggerCount } = effect;
      for (let occurrence = 0; occurrence < triggerCount; occurrence++) {
        layers.push({
          sourceInstanceId: source.instanceId,
          seat: link.attacker,
          // Negative indices identify engine-owned hook layers. Give repeated
          // occurrences distinct ordering ids when another on-hit trigger is
          // generated by the same event.
          triggerIndex: -2 - occurrence,
          label: "On hit",
          optional: false,
