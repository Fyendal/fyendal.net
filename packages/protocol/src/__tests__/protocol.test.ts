...
describe("client messages", () => {
  const variants = [
    ...
    { type: "background-match-accept", roomCode: "ABC123" },
...
describe("GameView and server messages", () => {
  ...
  it("accepts every server variant", () => {
    const variants = [
      ...
      { type: "background-matchmaking", status: { state: "inactive" } },
      { type: "background-matchmaking", status: { state: "pending", format: "cc" } },
      { type: "background-matchmaking", status: { state: "searching", format: "silver-age" } },
      { type: "background-matchmaking", status: {
        state: "offer",
        format: "cc",
        roomCode: "ABC123",
        deadlineAt: 30_000,
        opponent: { username: "bob", heroId: "HERO1", heroName: "Bob Hero" },
        acceptedByYou: false,
    ...
    expect(decodeAccountExportResponse({
      ...
      export: {
        ...
        matchmaking: {
          ...
          joinedAt: 2,
          mode: "background",
          sourceRoomCode: "BOT123",
          pendingOfferRoomCode: "ABC123",
          ...
          pendingBotStart: {
            ...
            cardPoolMode: "legal",
            requestedAt: 2,
            candidates: [{ candidateUserId: 2, ordinal: 0, attempted: false, skipped: false }],
          },
          offer: { roomCode: "ABC123", opponentUserId: 2, createdAt: 3 },
        },
        bugReports: [{
          id: "report-id",
          roomCode: "ABC123",
