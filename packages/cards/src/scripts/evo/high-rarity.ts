...
import { attackAbility, buffNextAttack, contractWithSilver, opponentSeat, requestDiscardChoice, resolveDiscardChoice } from "../shared-helpers.js";
...
function recordFabricateMode(ctx: ScriptCtx, mode: string) {
  ...
  if (used < 2) {
    ctx.requestChoice(
      "fabricate-mode",
      "Choose the second Fabricate mode",
...
function requestSteelControllerChoice(ctx: ScriptCtx) {
  ...
  ctx.requestCardChoice(
    "steel-controller",
    "Put an attack fifth from top?",
    ["no", ...cards.map((card) => card.instanceId)],
  );
}
function chooseItem(ctx: ScriptCtx, hook: string, prompt: string, optional = false) {
  const items = ctx.player(ctx.seat).board.filter((card) => isItem(ctx, card));
  if (items.length) ctx.requestCardChoice(hook, prompt, [...(optional ? ["no"] : []), ...items.map((card) => card.instanceId)]);
}
function galvanize(extra: CardScript = {}): CardScript {
  return {
    ...extra,
    onDefend(ctx) { chooseItem(ctx, "evo-galvanize", "Destroy an item for +2 defense?", true); extra.onDefend?.(ctx); },
...
function scrap(extra: CardScript = {}): CardScript {
  return {
    ...extra,
    additionalCost(ctx) { const cards = ctx.player(ctx.seat).graveyard.filter((card) => isItem(ctx, card)); if (cards.length) ctx.requestCardChoice("evo-scrap", "Banish an item from your graveyard", cards.map((card) => card.instanceId)); extra.additionalCost?.(ctx); },
...
function breakerEvo(extra: CardScript = {}): CardScript {
  return evoEquipment({
    ...
    additionalCost(ctx) {
      ...
      if (drivers.length) ctx.requestCardChoice("breaker-driver", "Choose Hyper Drivers to transform", ["done", ...drivers.map((card) => card.instanceId)]);
    ...
    onChoose(ctx, hook, option) {
      if (hook === "breaker-driver" && option !== "done") {
        ...
        if (drivers.length) ctx.requestCardChoice("breaker-driver", "Choose another Hyper Driver?", ["done", ...drivers.map((card) => card.instanceId)]);
      } else extra.onChoose?.(ctx, hook, option);
    },
    onEnterArena(ctx) {
...
export const evoHighRarity: Record<string, CardScript> = {
  ...
  "terminator tank|1": { ...evoThreshold((ctx) => { if (equippedEvos(ctx) >= 1) requestDiscardChoice(ctx, "terminator-discard", "Choose a card to discard", opponentSeat(ctx)); }), onChoose(ctx, hook, option) { if (hook === "terminator-discard") resolveDiscardChoice(ctx, option, opponentSeat(ctx)); } },
  ...
  "demolition protocol|1": { onAttackDeclared(ctx) { ctx.setCounter("upgrades", equippedEvos(ctx)); const cards = [...ctx.player(opponentSeat(ctx)).board, ...ctx.player(opponentSeat(ctx)).weapons, ...Object.values(ctx.player(opponentSeat(ctx)).equipment).filter((card): card is DeepReadonly<CardInstance> => !!card)].filter((card) => (card.counters?.steam ?? 0) > 0); if (cards.length && equippedEvos(ctx) > 0) ctx.requestCardChoice("demolition", "Remove steam counters from up to X permanents", ["done", ...cards.map((card) => card.instanceId)]); }, onChoose(ctx, hook, option) { if (hook !== "demolition" || option === "done") return; ctx.setCardCounter(Number(option), "steam", 0); const n = ctx.getCounter("demolished") + 1; ctx.setCounter("demolished", n); const cards = [...ctx.player(opponentSeat(ctx)).board, ...ctx.player(opponentSeat(ctx)).weapons, ...Object.values(ctx.player(opponentSeat(ctx)).equipment).filter((card): card is DeepReadonly<CardInstance> => !!card)].filter((card) => (card.counters?.steam ?? 0) > 0); if (n < ctx.getCounter("upgrades") && cards.length) ctx.requestCardChoice("demolition", "Remove steam from another permanent?", ["done", ...cards.map((card) => card.instanceId)]); } },
  ...
  "hyper scrapper|3": { variablePlayCost: { base: 0, counterKey: "scrapperX", prompt: "Choose X", maximum(ctx) { return ctx.player(ctx.seat).graveyard.filter((card) => isItem(ctx, card)).length; } }, additionalCost(ctx) { if (ctx.getCounter("scrapperX") <= 0) return; const items = ctx.player(ctx.seat).graveyard.filter((card) => isItem(ctx, card)); if (items.length) ctx.requestCardChoice("hyper-scrapper-item", "Banish an item from your graveyard", items.map((card) => card.instanceId)); }, onChoose(ctx, hook, option) { if (hook !== "hyper-scrapper-item") return; const card = ctx.player(ctx.seat).graveyard.find((candidate) => candidate.instanceId === Number(option)); if (card && ctx.banish(card.instanceId)) { ctx.addCounter(ctx.self.instanceId, "scrapped", 1); if (named(ctx, card, "hyper driver")) ctx.addCounter(ctx.self.instanceId, "scrappedDrivers", 1); } if (ctx.getCounter("scrapped") >= ctx.getCounter("scrapperX")) return; const items = ctx.player(ctx.seat).graveyard.filter((candidate) => isItem(ctx, candidate)); if (items.length) ctx.requestCardChoice("hyper-scrapper-item", "Banish another item from your graveyard", items.map((card) => card.instanceId)); }, modifyAttack: (ctx) => ctx.getCounter("scrapperX"), onAttackDeclared(ctx) { if (ctx.getCounter("scrappedDrivers") >= 3) { ctx.changeResources(ctx.seat, 6); ctx.grantGoAgain(); } } },
  "scrap trader|1": scrap({ onAttackDeclared(ctx) { ctx.changeResources(ctx.seat, ctx.getCounter("scrapped") * 2); }, onChoose() {} }),
  "moonshot|2": { variablePlayCost: { base: 0, resourcesPerX: 2, counterKey: "moonshotX", prompt: "Choose X", maximum(ctx) { return ctx.player(ctx.seat).board.filter((card) => named(ctx, card, "hyper driver")).length; } }, canPlay: (ctx) => ctx.getPlayerFlag(ctx.seat, "boostedThisTurn") === true, additionalCost(ctx) { if (ctx.getCounter("moonshotX") <= 0) return; const drivers = ctx.player(ctx.seat).board.filter((card) => named(ctx, card, "hyper driver")); if (drivers.length) ctx.requestCardChoice("moonshot", "Destroy a Hyper Driver", drivers.map((card) => card.instanceId)); }, onChoose(ctx, hook, option) { if (hook !== "moonshot") return; if (ctx.destroyPermanent(Number(option))) ctx.addCounter(ctx.self.instanceId, "drivers", 1); if (ctx.getCounter("drivers") >= ctx.getCounter("moonshotX")) return; const drivers = ctx.player(ctx.seat).board.filter((card) => named(ctx, card, "hyper driver")); if (drivers.length) ctx.requestCardChoice("moonshot", "Destroy another Hyper Driver", drivers.map((card) => card.instanceId)); }, modifyAttack: (ctx) => ctx.getCounter("drivers") * 3, onAttackDeclared(ctx) { if (ctx.currentAttackPower() >= 10) ctx.setFlag("link", "overpower", true); } },
  ...
  "meganetic lockwave|3": { variablePlayCost: { base: 0, resourcesPerX: 3, counterKey: "lockwaveX", prompt: "Choose X", maximum(ctx) { return Object.values(ctx.player(opponentSeat(ctx)).equipment).filter((card) => !!card).length; } }, playTargetOptions(ctx) { return [ctx.player(opponentSeat(ctx)).hero.instanceId]; }, onPlay(ctx) { if (ctx.getCounter("lockwaveX") <= 0) return; const equipment = Object.values(ctx.player(opponentSeat(ctx)).equipment).filter((card): card is DeepReadonly<CardInstance> => !!card); if (equipment.length) ctx.requestCardChoice("lockwave-offer", "Choose equipment to offer", equipment.map((card) => card.instanceId), opponentSeat(ctx)); }, onChoose(ctx, hook, option) { if (hook === "lockwave-offer") { const count = ctx.getCounter("lockwaveOffered") + 1; ctx.setCounter("lockwaveOffered", count); ctx.setCounter(`lockwaveOffer:${count}`, Number(option)); if (count < ctx.getCounter("lockwaveX")) { const chosen = new Set(Array.from({ length: count }, (_, index) => ctx.getCounter(`lockwaveOffer:${index + 1}`))); const equipment = Object.values(ctx.player(opponentSeat(ctx)).equipment).filter((card): card is DeepReadonly<CardInstance> => !!card && !chosen.has(card.instanceId)); if (equipment.length) ctx.requestCardChoice("lockwave-offer", "Choose another equipment to offer", equipment.map((card) => card.instanceId), opponentSeat(ctx)); return; } const offered = Array.from({ length: count }, (_, index) => ctx.getCounter(`lockwaveOffer:${index + 1}`)); ctx.requestCardChoice("lockwave-pick", "Choose equipment that must defend", offered); return; } if (hook === "lockwave-pick") ctx.setPlayerFlag(opponentSeat(ctx), `mustDefend:${option}`, true); } },
  ...
  "system reset|2": { variablePlayCost: { base: 0, counterKey: "systemResetX", prompt: "Choose X", maximum(ctx) { return ctx.player(ctx.seat).board.filter((card) => isItem(ctx, card) && has(ctx, card, "mechanologist") && (data(ctx, card).cost ?? 0) <= 1).length; } }, onPlay(ctx) { if (ctx.getCounter("systemResetX") <= 0) return; const items = ctx.player(ctx.seat).board.filter((card) => isItem(ctx, card) && has(ctx, card, "mechanologist") && (data(ctx, card).cost ?? 0) <= 1); if (items.length) ctx.requestCardChoice("system-reset", "Choose an item to reset", items.map((card) => card.instanceId)); }, onChoose(ctx, hook, option) { if (hook !== "system-reset") return; if (ctx.banish(Number(option))) { const count = ctx.getCounter("systemResetChosen") + 1; ctx.setCounter("systemResetChosen", count); ctx.setCounter(`systemResetItem:${count}`, Number(option)); } const chosen = ctx.getCounter("systemResetChosen"); if (chosen < ctx.getCounter("systemResetX")) { const items = ctx.player(ctx.seat).board.filter((card) => isItem(ctx, card) && has(ctx, card, "mechanologist") && (data(ctx, card).cost ?? 0) <= 1); if (items.length) ctx.requestCardChoice("system-reset", "Choose another item to reset", items.map((card) => card.instanceId)); return; } for (let index = 1; index <= chosen; index++) ctx.settleCard(ctx.getCounter(`systemResetItem:${index}`)); } },
  "fabricate|1": {
    additionalCost(ctx) {
      ctx.requestChoice("fabricate-mode", "Choose the first Fabricate mode", [...FABRICATE_MODES]);
    },
    onPlay(ctx) {
      ...
      if (ctx.getCounter("fabricate-mode:under") > 0) {
        ...
        if (evos.length) {
          ...
          ctx.requestCardChoice(
            "fabricate-under",
            "Choose an Evo equipment to put Fabricate under",
  ...
  "tectonic rift|3": { variablePlayCost: { base: 0, counterKey: "tectonicX", prompt: "Choose X" }, onPlay(ctx) { ctx.createTokens(SEISMIC, ctx.getCounter("tectonicX")); } },
  ...
  "sonata fantasmia|3": { variablePlayCost: { base: 0, resourcesPerX: 2, counterKey: "sonataX", prompt: "Choose X" }, onPlay(ctx) { const x = ctx.getCounter("sonataX"); ctx.createTokens(RUNECHANT, x); if (x >= 6) ctx.discardRandom(opponentSeat(ctx), 3); } },
