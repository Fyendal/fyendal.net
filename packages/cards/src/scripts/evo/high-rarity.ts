...
function scrap(extra: CardScript = {}, count = 1): CardScript {
  return {
    ...extra,
    declareAdditionalCost(ctx) {
      const cards = ctx.player(ctx.seat).graveyard.filter((card) => isItem(ctx, card) || data(ctx, card).cardType === "equipment");
      if (cards.length) ctx.requestCardChoice("evo-scrap", decisionPrompt("Scrap an item or equipment from your graveyard?", "card.evo.scrap.choose", { optionMessages: commonOptionMessages("none") }), ["none", ...cards.map((card) => card.instanceId)]);
      extra.declareAdditionalCost?.(ctx);
    },
    onChoose(ctx, hook, option) {
      if (hook !== "evo-scrap") { extra.onChoose?.(ctx, hook, option); return; }
      if (option === "none") return;
      const card = ctx.player(ctx.seat).graveyard.find((candidate) => candidate.instanceId === Number(option) && (isItem(ctx, candidate) || data(ctx, candidate).cardType === "equipment"));
      if (!card || !ctx.banish(card.instanceId)) return;
      const scrapped = ctx.getCounter("scrapped") + 1;
      ctx.setCounter("scrapped", scrapped);
      if (scrapped >= count) return;
      const cards = ctx.player(ctx.seat).graveyard.filter((candidate) => isItem(ctx, candidate) || data(ctx, candidate).cardType === "equipment");
      if (cards.length) ctx.requestCardChoice("evo-scrap", decisionPrompt("Scrap another item or equipment from your graveyard?", "card.evo.scrap.choose.next", { optionMessages: commonOptionMessages("none") }), ["none", ...cards.map((candidate) => candidate.instanceId)]);
    },
  };
}
...
export const evoHighRarity: Record<string, CardScript> = {
  ...
  "teklovossen, esteemed magnate|0": { allowsFriendlyCardPlayFrom: (ctx, card, zone) => zone === "banish" && has(ctx, card, "evo"), activated: { cost: 3, isAttack: false, goAgain: false, timing: "instant", oncePerTurn: true, label: "Play next Evo as an instant", onActivate(ctx) { ctx.setPlayerFlag(ctx.seat, "nextEvoAsInstant", true); ctx.setPlayerFlag(ctx.seat, "teklovossenNextEvoDraw", true); } }, triggers: [{ event: "card-played", label: "Draw a card", condition: (ctx, card) => !!card && has(ctx, card, "evo") && ctx.getPlayerFlag(ctx.seat, "teklovossenNextEvoDraw") === true, onTrigger(ctx) { ctx.setPlayerFlag(ctx.seat, "teklovossenNextEvoDraw", false); }, effect(ctx) { ctx.drawCards(ctx.seat, 1); } }] },
