...
function fusion(type: "earth" | "ice" | "lightning"): Pick<CardScript, "additionalCost" | "onChoose"> {
  return {
    additionalCost(ctx) {
      ...
      if (cards.length) ctx.requestCardChoice(`high-fuse:${type}`, `Reveal a ${type} card to fuse?`, ["no", ...cards.map((card) => card.instanceId)]);
    },
...
function elementalFusion(firstType: "earth" | "ice", secondType: "ice" | "lightning"): Pick<CardScript, "additionalCost" | "onChoose"> {
  return {
    additionalCost(ctx) {
      ...
      if (options.size > 1) ctx.requestChoice("elemental-fusion", `Reveal ${firstType} and/or ${secondType} cards to fuse?`, [...options]);
    },
    onChoose(ctx, hook, option) {
  ...
  return {
    additionalCost(ctx) {
      ...
      ctx.requestCardChoice(
        "oaken-fuse-ice",
        "Reveal an Ice card to fuse Oaken Old?",
    ...
    onChoose(ctx, hook, option) {
      if (hook === "oaken-fuse-ice") {
        ...
        ctx.requestCardChoice(
          "oaken-fuse-earth",
          "Reveal an Earth card to fuse Oaken Old?",
...
function channel(type: "earth" | "ice" | "lightning", extra: CardScript): CardScript {
  function maintain(ctx: ScriptCtx): void {
    ...
    ctx.requestCardChoice("channel-bottom", `Put a ${type} card from pitch on the bottom`, cards.map((card) => card.instanceId));
  }
  return {
    ...extra,
    triggers: [
...
const oldhim: CardScript = {
  activated: { cost: 3, isAttack: false, goAgain: false, timing: "defense-reaction", oncePerTurn: true,
    ...
    onActivate(ctx) { if (ctx.getCounter("oldhimEarth")) ctx.preventNextDamage(ctx.seat, 2); if (ctx.getCounter("oldhimIce") && ctx.link && ctx.player(ctx.link.attacker).hand.length) ctx.requestCardChoice("adult-oldhim-top", "Put a card from hand on top of deck", ctx.player(ctx.link.attacker).hand.map((card) => card.instanceId), ctx.link.attacker); } },
  onChoose(ctx, hook, option) { if (hook === "adult-oldhim-top") ctx.putOnDeckTop(Number(option)); },
};

const lexi: CardScript = {
  activated: { cost: 0, isAttack: false, goAgain: true, oncePerTurn: true, canActivate: (ctx) => ctx.player(ctx.seat).arsenal.some((card) => card.faceDown), onActivate(ctx) { ctx.requestCardChoice("adult-lexi-flip", "Turn a face-down arsenal card face up", ctx.player(ctx.seat).arsenal.filter((card) => card.faceDown).map((card) => card.instanceId)); } },
...
export const eleHighRarity: Record<string, CardScript> = {
  "korshem, crossroad of elements|0": {
    global: true,
    onAnyHeroReveals(ctx, revealingSeat) { ctx.setCounter("revealingSeat", revealingSeat + 1); ctx.requestChoice("korshem-benefit", "Choose Korshem's benefit", ["resource", "life", "attack", "defense"], revealingSeat); },
  ...
  "blizzard|3": {
    ...
    onPlay(ctx) {
      ...
      if (!ctx.requestPayment(
        "blizzard-pay",
        "Pay 2 resources to prevent the attack losing go again?",
        2,
        target.attacker,
  ...
  "channel thunder steppe|2": channel("lightning", {
    triggers: [{ event: "card-played", label: "Pay 1 to give this action go again?", condition: (ctx, played) => !!played && ctx.hasCardType(played, "action"), effect(ctx, played) { if (!played) return; ctx.setCounter("thunderTarget", played.instanceId); ctx.requestPayment("thunder-steppe", "Pay 1 to give this action go again?", 1); } }],
  ...
  "rampart of the ram's head|0": { defendCost: 1, onDefend(ctx) { ctx.requestPayment("rampart-pay", "Pay 1 for +1 defense?", 1); }, onChoose(ctx, hook, option) { if (hook === "rampart-pay" && option === "paid") ctx.addCardTempDefense(ctx.self.instanceId, 1); } },
  "new horizon|0": {
