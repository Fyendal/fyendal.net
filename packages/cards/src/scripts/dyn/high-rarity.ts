...
export const dynHighRarity: Record<string, CardScript> = {
  ...
  "eradicate|2": { ...contractWithSilver((ctx, card) => ctx.cardColor(card) === 2), canTriggerOnHit: (ctx) => ctx.link?.targetAllyId === undefined, onHit(ctx) { const count = Math.max(0, ctx.link?.damage ?? 0); for (const card of ctx.player(opponentSeat(ctx)).deck.slice(0, count)) ctx.banish(card.instanceId); } },
