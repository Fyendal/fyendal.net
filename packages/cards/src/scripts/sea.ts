...
Object.assign(sea, {
  ...
  "cog in the machine|1": { onPlay(ctx: ScriptCtx) { createGoldenCog(ctx); createGoldenCog(ctx); tapChoice(ctx, "cog-machine", "Tap a cog to bottom this?", controlledCogs(ctx, false)); }, onChoose(ctx: ScriptCtx, hook: string, option: string) { if (hook === "cog-machine" && option !== "pass" && ctx.tap(Number(option))) ctx.putOnDeckBottom(ctx.self.instanceId); } },
