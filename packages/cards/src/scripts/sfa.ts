...
export const sfa: Record<string, CardScript> = {
  ...
  "fire that burns within|1": {
    ...
    // CR 7.2.3–7.2.4: the attack trigger is a layer players may respond to.
    // Check the hand on resolution, after e.g. Fai has returned a Flame.
    // https://rules.fabtcg.com/en/cr/07-combat/
    triggers: [{
      event: "attack-declared",
      sourceZone: "self",
      label: "Discard a Phoenix Flame to draw a card and get +2{p}",
      labelMessage: decisionMessage("card.sfa.flame.discard.draw.trigger", { amount: 2 }),
      effect(ctx) {
