...
Object.assign(mst, {
  ...
  "supercell|3": { variablePlayCost: { base: 0, counterKey: "supercellX", prompt: decisionPrompt("Choose X", "engine.decision.x.choose") }, onPlay(ctx) { const x = ctx.getCounter("supercellX"); const id = tokenNamed(ctx, "Hyper Driver"); if (id) { const driver = ctx.createToken(id); if (driver) ctx.setCardCounter(driver.instanceId, "steam", x); } } },
