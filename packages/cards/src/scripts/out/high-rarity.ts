...
function arsenalFromHand(ctx: ScriptCtx, hook: string, filter = (_card: DeepReadonly<CardInstance>) => true): void { const cards = ctx.player(ctx.seat).hand.filter(filter); if (cards.length) ctx.requestCardChoice(hook, "Choose a card for arsenal", ["no", ...cards.map((card) => card.instanceId)]); }
function chooseDiscard(ctx: ScriptCtx, hook: string, seat: number): void { const hand = ctx.player(seat).hand; if (hand.length) ctx.requestCardChoice(hook, "Choose a card to discard", hand.map((card) => card.instanceId), seat); }
...
function codexBloodrotChoice(ctx: ScriptCtx, seat: number): void {
  ...
  ctx.requestCardChoice(`codex-bloodrot:${seat}`, "Choose a card for arsenal", player.hand.map((card) => card.instanceId), seat);
}
...
export const outHighRarity: Record<string, CardScript> = {
  ...
  "shake down|1": {
    ...
    onHit(ctx) {
            ctx.requestChoice("shake-color", "Choose a color", ["red", "yellow", "blue"]);
    },
  ...
  "head leads the tail|1": { onAttackDeclared(ctx) { ctx.requestNameChoice("head-tail-name", "Name another card"); }, onChoose(ctx, hook, option) { if (hook === "head-tail-name" && option.toLowerCase() !== ctx.data.name.toLowerCase()) ctx.addModifier({ scope: "combat-chain", attack: 1, appliesToName: option.toLowerCase() }); } },
  "wander with purpose|2": { onHit(ctx) { const zeroes = ctx.player(ctx.seat).hand.filter((card) => (ctx.cardData(card.cardId).cost ?? -1) === 0); if (zeroes.length) ctx.requestCardChoice("wander-discard", "Discard a zero-cost card?", ["no", ...zeroes.map((card) => card.instanceId)]); }, onChoose(ctx, hook, option) { if (hook === "wander-discard") { if (option === "no" || !ctx.discardCard(ctx.seat, Number(option))) return; const combos = ctx.player(ctx.seat).deck.filter((card) => (ctx.cardData(card.cardId).keywords ?? []).some((keyword) => keyword.toLowerCase() === "combo")); if (combos.length) ctx.requestCardChoice("wander-search", "Choose a combo card", combos.map((card) => card.instanceId)); else ctx.shuffleDeck(); } else if (hook === "wander-search" && ctx.banish(Number(option))) ctx.shuffleDeck(); } },
  ...
  "visit the floating dojo|3": {
    onPlay(ctx) { const strikes = ctx.player(ctx.seat).graveyard.filter((card) => namedCard(ctx, card, "surging strike")); if (strikes.length) ctx.requestCardChoice("dojo-strike", "Choose a Surging Strike", strikes.map((card) => card.instanceId)); else { const combos = ctx.player(ctx.seat).graveyard.filter((card) => combo(ctx, card)); if (combos.length) ctx.requestCardChoice("dojo-combo", "Choose a combo card", combos.map((card) => card.instanceId)); } },
    onChoose(ctx, hook, option) { if (hook === "dojo-strike" || hook === "dojo-combo") { ctx.setCounter(hook === "dojo-strike" ? "dojoStrike" : "dojoCombo", Number(option)); ctx.requestChoice(`${hook}-position`, "Put it on top or bottom?", ["top", "bottom"]); } else if (hook.endsWith("-position")) { const isStrike = hook.startsWith("dojo-strike"); const id = ctx.getCounter(isStrike ? "dojoStrike" : "dojoCombo"); if (option === "top") ctx.putOnDeckTop(id); else ctx.putOnDeckBottom(id); if (isStrike) { const combos = ctx.player(ctx.seat).graveyard.filter((card) => combo(ctx, card) && card.instanceId !== id); if (combos.length) ctx.requestCardChoice("dojo-combo", "Choose a combo card", combos.map((card) => card.instanceId)); } } },
  },
  ...
  "trench of sunken treasure|0": { preventArcaneDamage: 1, activated: { cost: 0, isAttack: false, goAgain: false, timing: "instant", oncePerTurn: true, canActivate: (ctx) => ctx.player(ctx.seat).arsenal.some((card) => card.faceDown), onActivate(ctx) { const cards = ctx.player(ctx.seat).arsenal.filter((card) => card.faceDown); ctx.requestCardChoice("trench-bottom", "Choose a face-down arsenal card", cards.map((card) => card.instanceId)); } }, onChoose(ctx, hook, option) { if (hook === "trench-bottom" && ctx.putOnDeckBottom(Number(option))) ctx.changeResources(ctx.seat, 1); } },
    ...
    activated: {
      ...
      onActivate(ctx) {
        ...
        if (daggers.length > 0) {
          ctx.requestCardChoice(
            "flick-knives-dagger",
            "Flick Knives: choose a dagger to deal 1 damage, then destroy it",
  ...
  "vambrace of determination|0": { activated: { cost: 1, isAttack: false, goAgain: false, timing: "attack-reaction", oncePerTurn: true, onActivate(ctx) { ctx.setPlayerFlag(ctx.seat, "nextPhysicalPreventionReduction", 1); } }, onDefend(ctx) { ctx.requestPayment("vambrace", "Pay 1 for +1 defense and blade break?", 1); }, onChoose(ctx, hook, option) { if (hook === "vambrace" && option === "paid") { ctx.addCardTempDefense(ctx.self.instanceId, 1); ctx.grantCardKeyword(ctx.self.instanceId, "blade break"); } } },
