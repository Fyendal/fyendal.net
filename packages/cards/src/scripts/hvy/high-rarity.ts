...
import { attackAbility, buffNextAttack, commonOptionMessages, contractCompletionCount, decisionMessage, decisionPrompt, opponentSeat } from "../shared-helpers.js";
...
function chooseArsenalBottom(ctx: ScriptCtx, hook: string) {
  ...
  if (card) ctx.requestCardChoice(hook, decisionPrompt("Put your arsenal card on the bottom?", "card.hvy.arsenal.bottom", { optionMessages: commonOptionMessages("no") }), ["no", card.instanceId]);
}
...
function requestAnteMode(ctx: ScriptCtx): void {
  ...
  ctx.requestChoice(
    "ante-mode",
    decisionPrompt(remaining === total ? "Choose a wager mode" : "Choose another wager mode", remaining === total ? "card.hvy.wager.mode.choose" : "card.hvy.wager.mode.next", { optionMessages: {
      agility: decisionMessage("card.hvy.wager.option.agility"),
      gold: decisionMessage("card.hvy.wager.option.gold"),
      vigor: decisionMessage("card.hvy.wager.option.vigor"),
      power: decisionMessage("card.hvy.wager.option.power"),
    } }),
...
export const hvyHighRarity: Record<string, CardScript> = {
  ...
  "kassai of the golden sand|0": { modifyAttackActivationCost(ctx, attacker, base) { return has(ctx, attacker, "sword") && Number(ctx.getPlayerFlag(ctx.seat, "cardsDrawnThisTurn")) > 0 ? Math.max(0, base - 1) : base; }, activated: { cost: 0, isAttack: false, goAgain: true, oncePerTurn: true, effectCardCosts: [{ zone: "graveyard", move: "banish", count: 2, pitch: 1, prompt: decisionPrompt("Banish 2 red cards", "card.common.cost.red.banish.two") }, { zone: "graveyard", move: "banish", count: 2, pitch: 2, prompt: decisionPrompt("Banish 2 yellow cards", "card.common.cost.yellow.banish.two") }], label: "Next weapon hit creates Gold", onActivate(ctx) { buffNextAttack(ctx, { appliesTo: "weapon", onHitCreateToken: { cardId: GOLD, count: 1 } }); } } },
  "grains of bloodspill|0": { canTriggerOnHit: (ctx) => ctx.link?.attackCardType === "weapon", onHit(ctx) { ctx.requestPayment("grains", decisionPrompt("Pay 1 to create Vigor?", "card.hvy.vigor.pay", { values: { amount: 1 }, optionMessages: commonOptionMessages("no") }), 1); }, onChoose(ctx, hook, option) { if (hook === "grains" && option === "paid") ctx.createToken(VIGOR); } },
  ...
  "up the ante|3": { variablePlayCost: { base: 0, maximum: ANTE_MODES.length - 1, counterKey: "anteX", prompt: decisionPrompt("Choose X", "engine.decision.x.choose") }, playTargetOptions(ctx) { return ctx.link ? [ctx.link.attackingCard.instanceId] : []; }, additionalCost(ctx) { ctx.setCounter("ante-remaining", ctx.getCounter("anteX") + 1); requestAnteMode(ctx); }, onPlay(ctx) { applyAnteModes(ctx); }, onChoose(ctx, hook, option) { if (hook !== "ante-mode" || !ANTE_MODES.includes(option as AnteMode)) return; const mode = option as AnteMode; ctx.setCounter(`ante-mode:${mode}`, 1); ctx.setCounter("ante-remaining", ctx.getCounter("ante-remaining") - 1); requestAnteMode(ctx); } },
  ...
  "talk a big game|3": { onPlay(ctx) { ctx.addModifier({ scope: "until-end-of-turn" }); ctx.requestChoice("big-number", decisionPrompt("Choose a number", "card.hvy.number.choose"), ["1", "2", "3", "4", "5", "6"]); }, onChoose(ctx, hook, option) { if (hook === "big-number") ctx.setCounter("talk-threshold", Number(option)); }, onFriendlyDamageDealt(ctx, _source, target, amount, arcane) { talkDamage(ctx, target, amount, arcane); }, onFriendlyCombatDamageDealt(ctx, _source, target, amount) { talkDamage(ctx, target, amount, false); } },
  ...
  "graven call|0": { activated: [{ ...attackAbility(2, { goAgain: true })[0]!, label: "Attack" }, { cost: 0, isAttack: false, goAgain: false, timing: "instant", fromGraveyard: true, effectCardCosts: [{ zone: "arena", move: "destroy", count: 2, name: "Silver", prompt: decisionPrompt("Destroy 2 Silver", "card.common.cost.silver.destroy.two") }], label: "Equip from graveyard", onActivate(ctx) { if (ctx.equipFromGraveyard(ctx.self.instanceId)) ctx.addCounter(ctx.self.instanceId, "power", 1); } }] },
  "coercive tendency|3": { onPlay(ctx) { const cards = ctx.player(opponentSeat(ctx)).deck.slice(0, 3); for (const card of cards) ctx.lookAt(card.instanceId); if (cards.length) ctx.requestCardChoice("coercive-first", decisionPrompt("Choose the top card", "card.hvy.deck.top.choose"), cards.map((card) => card.instanceId)); }, onChoose(ctx, hook, option, _unused?: never) { if (hook === "coercive-first") { ctx.setCounter("coercive-first", Number(option)); const cards = ctx.player(opponentSeat(ctx)).deck.slice(0, 3).filter((card) => card.instanceId !== Number(option)); if (cards.length) ctx.requestCardChoice("coercive-second", decisionPrompt("Choose the second card", "card.hvy.deck.second.choose"), cards.map((card) => card.instanceId)); return; } if (hook !== "coercive-second") return; const first = ctx.getCounter("coercive-first"); const cards = ctx.player(opponentSeat(ctx)).deck.slice(0, 3); const third = cards.find((card) => card.instanceId !== first && card.instanceId !== Number(option)); if (third) ctx.putOnDeckTop(third.instanceId); ctx.putOnDeckTop(Number(option)); ctx.putOnDeckTop(first); const completions = contractCompletionCount(ctx); const top = ctx.player(opponentSeat(ctx)).deck[0]; if (top) ctx.banish(top.instanceId); if (contractCompletionCount(ctx) > completions) { ctx.addModifier({ scope: "combat-chain", goAgain: true, appliesTo: "attack", appliesToClass: "assassin" }); if (ctx.link?.attacker === ctx.seat && has(ctx, ctx.link.attackingCard, "assassin")) ctx.grantGoAgain(); } } },
