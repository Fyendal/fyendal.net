...
function rawDecisionPrompts(set: string): string[] {
  const path = `${scriptsDirectory}/${set}.ts`;
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
      ...
      if (prompt && (
        ts.isStringLiteral(prompt) ||
        ts.isNoSubstitutionTemplateLiteral(prompt) ||
        ts.isTemplateExpression(prompt)
      )) {
        const position = sourceFile.getLineAndCharacterOfPosition(prompt.getStart(sourceFile));
        raw.push(`${set}.ts:${position.line + 1}`);
      }
    }
